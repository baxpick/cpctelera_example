...
public:
  ...
  Data(unsigned char *n_data,int n_size) : _data(0), _size(n_size), _maxsize(n_size) { _resize(n_size); memcpy(_data,n_data,n_size); }
  ~Data() { delete [] _data; }
  void setdata(unsigned char *buffer, int size) { _data=new unsigned char[size]; memcpy(_data,buffer,size); _size=_maxsize=size; }
  void push(unsigned char n_byte) { if (_size>=_maxsize) _grow(_size); _data[_size]=n_byte; ++_size; }
  ...
  unsigned char operator[](int index) const { return _data[index]; }
  unsigned char &operator[](int index) { if (index>=_maxsize) _grow(index); if (_size<=index) _size=index+1; return _data[index]; }
  int size() { return _size; }
  void clear() { _size=0; }
  unsigned char *getdatap(int offset=0) { return _data+offset; }
